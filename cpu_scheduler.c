...
void firstComeFirstServe(Process processes[], int count)
{
    processes[0].start = 0;
    processes[0].finish = processes[0].cpuburst + processes[0].arrival;
    for (int i = 1; i < count; i++)
    {
        processes[i].start = processes[i - 1].finish;
        processes[i].finish = processes[i].start + processes[i].cpuburst;
    }
    for (int i = 0; i < count; i++)
    {
        processes[i].turnaround = maximum(0, processes[i].finish - processes[i].arrival);
        processes[i].wait = maximum(0, processes[i].turnaround - processes[i].cpuburst);
    }
    PrintStatistics1(processes, count);
}
...
void srbfWaiting(Process processes[], int record, int wait[])
{
    ...
    while (complete != record)
    {
        ...
        if (rt[shortest] == 0)
        {
            ...
            finish_time = t + 1;
            wait[shortest] = finish_time -
                             processes[shortest].cpuburst -
...
int main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        read = fscanf(file, "%5[^,],%d,%d",
                      processes[record].name,
                      &processes[record].arrival,
                      &processes[record].cpuburst);
    ...
    while (true)
    {
        ...
        {
        case 1:
            firstComeFirstServe(processes, record);
            printf("\n");
